...
Mapping_sheets = list(pd.read_excel(path_mapping + '/OSeMOSYS_mapping_for_2020_data.xlsx', sheet_name = None).keys())[1:]

Mapping_file = pd.DataFrame()

for sheet in Mapping_sheets:
    interim_map = pd.read_excel(path_mapping + '/OSeMOSYS_mapping_for_2020_data.xlsx', sheet_name = sheet, skiprows = 1)
...
# Lowest level
industry_agg = ['14_3_1_iron_and_steel', '14_3_2_chemical_incl_petrochemical', '14_3_3_non_ferrous_metals',
               '14_3_4_nonmetallic_mineral_products', '14_3_5_transportation_equipment', '14_3_6_machinery', '14_1_mining_and_quarrying',
               '14_3_7_food_beverages_and_tobacco', '14_3_8_pulp_paper_and_printing', '14_3_9_wood_and_wood_products', 
               '14_2_construction', '14_3_10_textiles_and_leather', '14_3_11_nonspecified_industry']
...
# Then first level
tpes_agg = ['1_production', '2_imports', '3_exports', '4_international_marine_bunkers', '5_international_aviation_bunkers', '6_stock_changes']
...
EGEDA_years = pd.read_csv('./data/1_EGEDA/EGEDA_2020_created_21102022.csv')
...
if ref_aggregate_df2_tojoin.empty == True:
    Joined_ref_df = EGEDA_years.copy().reindex(columns = EGEDA_years.columns.tolist() + list(range(2021, 2051)))
else:
    Joined_ref_df = EGEDA_years.copy().iloc[:, :].merge(ref_aggregate_df2_tojoin[key_variables + list(ref_aggregate_df2_tojoin.iloc[:, 7:])],\
        on = ['economy', 'fuel_code', 'item_code_new'], how = 'left')

Joined_ref_df.to_csv(path_final + '/OSeMOSYS_to_EGEDA_2020_reference.csv', index = False)

# NET ZERO
if netz_aggregate_df2_tojoin.empty == True:
    Joined_netz_df = EGEDA_years.copy().reindex(columns = EGEDA_years.columns.tolist() + list(range(2021, 2051)))
else:
    Joined_netz_df = EGEDA_years.copy().iloc[:, :].merge(netz_aggregate_df2_tojoin[key_variables + list(ref_aggregate_df2_tojoin.iloc[:, 7:])],\
        on = ['economy', 'fuel_code', 'item_code_new'], how = 'left')

Joined_netz_df.to_csv(path_final + '/OSeMOSYS_to_EGEDA_2020_netzero.csv', index = False)
