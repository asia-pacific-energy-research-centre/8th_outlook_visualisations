...
for economy in Economy_codes:
    ...
    script_dir = './results/'
    results_dir = os.path.join(script_dir, 'APEC_regional_II')
    ...
    # FED
    ref_bld_2.loc[ref_bld_2['fuel_code'] == 'Total', 'fuel_code'] = 'Buildings_total_ref'
    ref_ind_2.loc[ref_ind_2['fuel_code'] == 'Total', 'fuel_code'] = 'Industry_total_ref'
    ref_trn_1.loc[ref_trn_1['fuel_code'] == 'Total', 'fuel_code'] = 'Transport_total_ref'
    netz_bld_2.loc[netz_bld_2['fuel_code'] == 'Total', 'fuel_code'] = 'Buildings_total_cn'
    netz_ind_2.loc[netz_ind_2['fuel_code'] == 'Total', 'fuel_code'] = 'Industry_total_cn'
    netz_trn_1.loc[netz_trn_1['fuel_code'] == 'Total', 'fuel_code'] = 'Transport_total_cn'

    # Select relevant dataframes to put in sheets
    ref_bld_2.to_excel(writer, sheet_name = economy + ' df II', index = False, startrow = 3)
    ref_ind_2.to_excel(writer, sheet_name = economy + ' df II', index = False, startrow = ref_bld_2_rows + 6)
    ref_trn_1.to_excel(writer, sheet_name = economy + ' df II', index = False, startrow = ref_bld_2_rows + ref_ind_2_rows + 9)
    netz_bld_2.to_excel(writer, sheet_name = economy + ' df II', index = False, startrow = ref_bld_2_rows + ref_ind_2_rows + ref_trn_1_rows + 12)
    netz_ind_2.to_excel(writer, sheet_name = economy + ' df II', index = False, startrow = ref_bld_2_rows + ref_ind_2_rows + ref_trn_1_rows + netz_bld_2_rows + 15)
    netz_trn_1.to_excel(writer, sheet_name = economy + ' df II', index = False, startrow = ref_bld_2_rows + ref_ind_2_rows + ref_trn_1_rows + netz_bld_2_rows + netz_ind_2_rows + 18)
    ...
    ref_worksheet1 = writer.sheets[economy + ' df II']
    ...
    ref_worksheet1.set_column(1, ref_bld_2_cols + 1, None, space_format)
    #ref_worksheet1.set_row(chart_height, None, header_format)
    ref_worksheet1.write(0, 0, economy + ' select FED dataframes REF and CN', cell_format1)
...
excel_filenames = glob.glob("./results/APEC_regional_II/*.xlsx")

writer2 = pd.ExcelWriter('./results/APEC_regional_II/combined.xlsx', engine = 'xlsxwriter')

for file in excel_filenames:
    temp_1 = pd.read_excel(file)
    temp_1.to_excel(writer2, sheet_name = file[27:33], index = False)
